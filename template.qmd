...
how much training data to use (10 folds)
...
# Sample from the sequence of the 1 through 5, but it puts the sampled number back in
...
```{r}
total_model <- lm(f2, data = penguins_clean)
summary_model <- summary(total_model)
```
